...
- **전체 정확도 98.5%** — 4,555개 윈도우 중 4,487개 정답
- **이물질 재현율 100%** — 가장 위험한 이물질을 단 한 번도 놓치지 않음 (1,691/1,691)
- **정상 오탐률 2.4%** (33/1,357) · **부하 놓침 2.3%** (35/1,507) — 두 오류가 균형

> **경계값(9.4)의 근거:** 경계를 올리면 정상 오탐은 줄지만 부하(이상)를 놓치는 비율이 늘고,
> 내리면 반대가 됩니다. 정상 오탐(거짓 경보)과 부하 놓침(이상 놓침)이 균형을 이루는 9.4를
> 선택했습니다. 특히 예지보전에서 더 위험한 '이상 놓침'을 2.3%로 억제했습니다.
> (약 8분 분량 실측 데이터 기반이며, 다양한 환경으로의 일반화는 데이터 추가 확대와 함께
> 지속 검증할 과제입니다.)
