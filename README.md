...
> **어떤 성능으로** — 전류 기반 3-클래스 진단 **정확도 97.5%**, 이물질 **재현율 100%** (데이터 오염 정제 + 스파이크 분석으로 정상 오탐 개선, 실측 재현 검증)
...
- Python으로 정상/부하/이물질 데이터 분석 → 유형별 고유 특징 규명
  ...
  >
  > **데이터 정제:** 초기엔 정상 표준편차(2.70)가 이물질(2.28)보다 커서 물리적으로 이상했는데,
  > 원본을 추적하니 **주행 종료 후 전류가 0으로 떨어졌음에도 로깅이 마지막 명령(FORWARD)을
  > 유지한 채 25행을 더 기록한 '꼬리 0 블록'** 이 원인이었습니다. 이 구간을 제거하자 정상
  > 표준편차가 1.66으로 내려가, **이물질(2.28)이 가장 큰 변동**을 보이는 정상적인 순서가 됐습니다.
  > (마찰·이물질일수록 전류가 불규칙하게 튀어 변동이 크다는 설계 의도와 일치)
...
실측 CSV(정상/부하/이물질 각 FORWARD 구간, 꼬리 0 블록 제거)에 판정 로직을 적용해 **재현한** 결과입니다.
판정은 `current_diff`의 **슬라이딩 윈도우(W=20, 겹침)** 대표값을 경계값으로 분류합니다.
경계값은 각 상태 분포의 중간점을 근거로 설정했습니다 (정상/부하 = 9.5, 부하/이물질 = 12.0).
전체 재현 코드는 [`analysis/classify.py`](analysis/classify.py) 참조.

**오탐 원인 분석 → 개선.** 초기 '평균' 판정에서 정상 오탐(정상→부하) 4.9%가 발생했는데,
오탐 구간을 분석하니 **순간 전류 스파이크**(예: `0 → 16 → 21`)가 윈도우 평균을 경계 위로
밀어올린 것이 원인이었습니다. (배터리 방전 가설도 검토했으나, 데이터상 정상 구간에서만
전류가 완만히 하락해 오탐과는 무관함을 확인.) 스파이크에 강건한 **중앙값(median)** 으로
대표값을 교체하여 정상 오탐을 **4.9% → 1.5%**, 전체 정확도를 **96.4% → 97.5%** 로 개선했습니다.

**혼동 행렬** (중앙값 판정, 행: 실제 / 열: 예측, 단위: 윈도우 개수)
...
- **전체 정확도 97.5%** — 1,119개 윈도우 중 1,091개 정답
- **이물질 재현율 100%** — 가장 위험한 이물질을 단 한 번도 놓치지 않음 (467/467)
- **정상 오탐률 1.5%** — 정상을 이상으로 잘못 알린 비율 (5/327)

> **경계값 선택의 근거:** 경계를 9.6으로 올리면 정상 오탐을 0%까지 낮출 수 있으나,
> 그 경우 부하(이상)를 정상으로 놓치는 비율이 증가합니다. 예지보전에서는 '거짓 경보'보다
> '이상을 놓치는 것'이 더 위험하므로, 전체 정확도가 가장 높고 이상 놓침이 최소인 9.5를
> 선택했습니다. (약 2분 분량 실측 데이터 기반 개념 검증(PoC)이며, 정량 성능의 일반화는
> 데이터 확대와 함께 향후 과제입니다.)
